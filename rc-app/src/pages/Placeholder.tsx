  ...
  return (
    <Shell crumb={["风控", label]}>
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 text-center">
        ...
        <div><h2 className="text-[18px] font-bold">{label} · 建设中</h2><p className="mt-1.5 max-w-md text-[13px] text-muted-foreground">本期先交付「交易警报」模块。该模块为占位,后续按设计图依次补齐。</p></div>
        <Button style={{ background: "var(--brand)" }} onClick={() => nav("/alerts")}>前往交易警报</Button>
      </div>
    </Shell>
  );
}