...
export function ReviewDialog({ alertId, open, onOpenChange, onDone }: { alertId: string | null; open: boolean; onOpenChange: (o: boolean) => void; onDone?: () => void }) {
  ...
  const submit = () => {
    ...
    // 建案 / 拒绝 → 汇入案件管理(同主体在办则并入,否则新建;案件 = 唯一调查容器)
    if (choice === "case" || choice === "reject") {
      const risk = a.sanctions.status === "直接命中" ? "制裁规避" : "可疑洗钱";
      const { id, attached } = intakeCase({ subject: a.merchant, sub: `${a.country} · 商户`, type: a.title, risk, amount: a.amount, src: gate ? "事中驳回" : "告警升级", linkIds: `${a.id} · ${a.order}`, linkTo: `/alert?id=${a.id}`, addSubject: { name: a.merchant, type: "商户", role: `${gate ? "事中驳回" : "告警升级"} · ${a.title}`, amount: a.amount } });
      toast(attached ? `已并入在办案件 ${id} · 同主体统一调查` : `已建案 ${id} · 在案件管理确认可疑后起草 STR`);
    }
