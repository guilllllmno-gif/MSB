...
export const RINGS: Ring[] = [
  { id: "RING-0001", name: "混币器归集网络", method: "分层洗钱", confidence: "high", confidenceScore: 92, memberCount: 2, walletCount: 5, totalAmountCad: 124000, overLctr: true, alertCount: 6, status: "investigating", assignee: { name: "Sarah Chen", initials: "SC" }, slaRemaining: "剩 18h", slaUrgent: false, submittedAt: "2026-03-16",
  ...
  { id: "RING-0002", name: "众包养卡入金网络", method: "结构化拆分", confidence: "high", confidenceScore: 88, memberCount: 4, walletCount: 11, totalAmountCad: 38400, overLctr: true, alertCount: 9, status: "escalated", assignee: { name: "David Wu", initials: "DW" }, slaRemaining: "剩 6h", slaUrgent: true, submittedAt: "2026-03-15",
  ...
  { id: "RING-0003", name: "跨境分层归集网络", method: "分层洗钱", confidence: "high", confidenceScore: 86, memberCount: 2, walletCount: 5, totalAmountCad: 124000, overLctr: true, alertCount: 5, status: "pending", assignee: null, slaRemaining: "剩 1d 02h", slaUrgent: false, submittedAt: "2026-03-17",
    hitDimensions: [ev("fund", "strong", 40, "归集后经中转分发出口"), ev("withdraw_addr", "weak", 20, "出口地址复用"), ev("device", "weak", 14, "设备群 DV-9")] },
  { id: "RING-0004", name: "拆分入金群组 #A7", method: "结构化拆分", confidence: "mid", confidenceScore: 66, memberCount: 3, walletCount: 7, totalAmountCad: 19800, overLctr: false, alertCount: 3, status: "watching", assignee: null, slaRemaining: null, slaUrgent: false, submittedAt: "2026-03-20",
    hitDimensions: [ev("fund", "weak", 24, "相近金额拆分入金"), ev("ip", "weak", 15, "同 IP 段登录")] },
  { id: "RING-0005", name: "快进快出过账链", method: "快速过水", confidence: "mid", confidenceScore: 71, memberCount: 3, walletCount: 6, totalAmountCad: 41200, overLctr: true, alertCount: 4, status: "pending", assignee: null, slaRemaining: "剩 20h", slaUrgent: false, submittedAt: "2026-03-19",
    hitDimensions: [ev("fund", "strong", 38, "入金后 1h 内全额转出"), ev("withdraw_addr", "weak", 18, "出口地址相近")] },
  { id: "RING-0006", name: "代付资金归集网络", method: "第三方代付", confidence: "mid", confidenceScore: 79, memberCount: 4, walletCount: 9, totalAmountCad: 56000, overLctr: true, alertCount: 5, status: "investigating", assignee: { name: "James Liu", initials: "JL" }, slaRemaining: "剩 2d", slaUrgent: false, submittedAt: "2026-03-14",
    hitDimensions: [ev("fund", "strong", 36, "多方代付归集"), ev("device", "weak", 16, "共享设备"), ev("ip", "weak", 9, "同 IP 段")] },
  { id: "RING-0007", name: "制裁实体关联网络", method: "制裁规避", confidence: "high", confidenceScore: 96, memberCount: 2, walletCount: 4, totalAmountCad: 88000, overLctr: true, alertCount: 7, status: "merged_case", assignee: { name: "Emma Zhang", initials: "EZ" }, slaRemaining: null, slaUrgent: false, submittedAt: "2026-03-13",
    hitDimensions: [ev("withdraw_addr", "strong", 40, "命中 OFAC 制裁地址"), ev("fund", "strong", 30, "资金注入制裁关联实体")] },
  { id: "RING-0008", name: "共用设备登录簇", method: "设备聚类", confidence: "low", confidenceScore: 42, memberCount: 3, walletCount: 5, totalAmountCad: 9600, overLctr: false, alertCount: 1, status: "closed", assignee: { name: "Sarah Chen", initials: "SC" }, slaRemaining: null, slaUrgent: false, submittedAt: "2026-02-28",
...
export function buildRingDetail(r: Ring): RingDetail {
  ...
  const members: RingMember[] = linked.length
    ? linked.map((s, i) => ({ subjectId: s.id, subjectName: s.name, avatar: s.avatar, color: PALETTE[i % PALETTE.length], role: s.ringRole ?? "团伙成员", walletCount: s.walletCount }))
    : Array.from({ length: Math.max(1, r.memberCount) }, (_, i) => ({ subjectId: `${r.id}-M${i + 1}`, subjectName: i === 0 ? "归集主体" : `关联主体 ${i + 1}`, avatar: i === 0 ? "核" : `M${i + 1}`, color: PALETTE[i % PALETTE.length], role: i === 0 ? "归集核心" : "关联主体", walletCount: 1 + ((h >>> i) % 3) }));
